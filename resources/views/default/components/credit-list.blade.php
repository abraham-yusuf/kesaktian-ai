...
// PERBAIKAN: Pastikan total tidak pernah 0 atau minus untuk mencegah error "Division by zero" atau "-INF"
if ($totalCreditsCount <= 0) {
    $totalCreditsCount = 1;
}
    ...
    {{-- PERBAIKAN: Tambahkan max() dan cast (int) untuk keamanan --}}
    style="width: {{ $wordContainUnlimited ? 100 : (int)(($wordCreditsCount / $totalCreditsCount) * 100) }}%"
></div>
<div
    {{ $attributes->twMergeFor('progressbar-image', $progressbar_image_base_class) }}
    style="width: {{ $imageContainUnlimited ? 100 : (int)(($imageCreditsCount / $totalCreditsCount) * 100) }}%"
></div>
